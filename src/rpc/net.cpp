...
static RPCHelpMan getpeerinfo()
{
    return RPCHelpMan{
        ...
        RPCResult{
            ...
            {{
                ...
                {{
                    ...
                    {RPCResult::Type::STR, "addrbind", /*optional=*/true,
                    ...
                    {RPCResult::Type::BOOL, "relaytxes", /*optional=*/true,
                    ...
                    {RPCResult::Type::NUM_TIME, "last_block",
                     "The " + UNIX_EPOCH_TIME + " of the last block received from this peer"},
                    {RPCResult::Type::NUM, "bytessent", "The total bytes sent"},
                    {RPCResult::Type::NUM, "bytesrecv", "The total bytes received"},
                    {RPCResult::Type::NUM_TIME, "conntime",
                     "The " + UNIX_EPOCH_TIME + " of the connection"},
                    {RPCResult::Type::NUM, "timeoffset", "The time offset in seconds"},
                    {RPCResult::Type::NUM, "pingtime", /*optional=*/true,
                    ...
                    {RPCResult::Type::NUM, "startingheight", /*optional=*/true,
                     "The starting height (block) of the peer"},
                    {RPCResult::Type::NUM, "presynced_headers", /*optional=*/true,
                     "The current height of header pre-synchronization with this peer, or -1 if no low-work sync is in progress"},
                    {RPCResult::Type::NUM, "synced_headers", /*optional=*/true,
                     "The last header we have in common with this peer"},
                    {RPCResult::Type::NUM, "synced_blocks", /*optional=*/true,
        ...
        RPCExamples{HelpExampleCli("getpeerinfo", "") + HelpExampleRpc("getpeerinfo", "")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            const CConnman& connman = EnsureConnman(node);
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (strCommand == "onetry") {
                CAddress addr;
                connman.OpenNetworkConnection(addr, false, nullptr, strNode.c_str(),
...
static RPCHelpMan addconnection()
{
    return RPCHelpMan{
        ...
        RPCExamples{HelpExampleCli("addconnection", "\"192.168.0.6:8333\" \"outbound-full-relay\"") + HelpExampleRpc("addconnection", "\"192.168.0.6:8333\" \"outbound-full-relay\"")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
...
static RPCHelpMan disconnectnode()
{
    return RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::DefaultHint{"fallback to nodeid"},
             "The IP address/port of the node"},
            {"nodeid", RPCArg::Type::NUM, RPCArg::DefaultHint{"fallback to address"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (!address_arg.isNull() && id_arg.isNull()) {
                ...
                success = connman.DisconnectNode(address_arg.get_str());
                       ...
                       (address_arg.isNull() || (address_arg.isStr() && address_arg.get_str().empty()))) {
...
static RPCHelpMan getaddednodeinfo()
{
    return RPCHelpMan{
        ...
        RPCExamples{HelpExampleCli("getaddednodeinfo", "\"192.168.0.201\"") + HelpExampleRpc("getaddednodeinfo", "\"192.168.0.201\"")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            const CConnman& connman = EnsureConnman(node);
...
static RPCHelpMan getnetworkinfo()
{
    return RPCHelpMan{
        ...
        RPCExamples{HelpExampleCli("getnetworkinfo", "") + HelpExampleRpc("getnetworkinfo", "")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            LOCK(cs_main);
            UniValue obj(UniValue::VOBJ);
            ...
            if (node.mempool) {
                ...
                obj.pushKV("incrementalfee",
                           ValueFromAmount(node.mempool->m_incremental_relay_feerate.GetFeePerK()));
            }
...
static RPCHelpMan listbanned()
{
    return RPCHelpMan{
        ...
        RPCExamples{HelpExampleCli("listbanned", "") + HelpExampleRpc("listbanned", "")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            if (!node.banman) {
                throw JSONRPCError(RPC_DATABASE_ERROR, "Error: Ban database not loaded");
...
static RPCHelpMan getnodeaddresses()
{
    return RPCHelpMan{
        ...
        RPCExamples{HelpExampleCli("getnodeaddresses", "8") + HelpExampleCli("getnodeaddresses", "4 \"i2p\"") + HelpExampleCli("-named getnodeaddresses", "network=onion count=12") + HelpExampleRpc("getnodeaddresses", "8") + HelpExampleRpc("getnodeaddresses", "4, \"i2p\"")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            const CConnman& connman = EnsureConnman(node);
            ...
            const std::optional<Network> network{
                request.params[1].isNull() ? std::nullopt : std::optional<Network>{ParseNetwork(request.params[1].get_str())}};
            if (network == NET_UNROUTABLE) {
                throw JSONRPCError(RPC_INVALID_PARAMETER,
                                   strprintf("Network not recognized: %s", request.params[1].get_str()));
            }
...
static RPCHelpMan addpeeraddress()
{
    return RPCHelpMan{
        ...
        RPCExamples{
            HelpExampleCli("addpeeraddress", "\"1.2.3.4\" 8333 true") + HelpExampleRpc("addpeeraddress", "\"1.2.3.4\", 8333, true")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            if (!node.addrman) {
...
static RPCHelpMan sendaddrmessage()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            const std::string& addr_string{request.params[1].get_str()};
            const auto port{request.params[2].getInt<uint16_t>()};
...
static RPCHelpMan sendgetaddrmessage()
{
    return RPCHelpMan{
        ...
        RPCResult{RPCResult::Type::NONE, "", ""},
        RPCExamples{
            HelpExampleCli("sendgetaddrmessage", "\"1.2.3.4:8333\"")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            NodeContext& node = EnsureAnyNodeContext(request.context);
            PeerManager& peerman = EnsurePeerman(node);
            CConnman& connman = EnsureConnman(node);
